...
你需要更改 **vlm_analyzer.py** 脚本中的提示词工程以适配你的下游任务目标。
...
# 未来改进：  
首先**数据安全问题**：要想真正解决私域数据，需要通过下载开源模型，实行LLMs的 **本地部署**，利用在本地完成推理比上传到第三方大模型中要稳妥的多。
其次**准确性问题**：虽然通过构建 提示词工程 来优化模型输出，但这只能从外部约束模型的推理。真正想要提升对于本领域的推理效果，依然需要借助 **本地部署＋下游适配(微调)** 的过程。

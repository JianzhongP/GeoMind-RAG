...
export function PreviewPanel({
  ...
  // 新增 添加用户小窗拖拽
  const [panelWidth, setPanelWidth] = useState(420); // 初始宽度
  const isResizing = useRef(false);
  // 在 useEffect 中监听全局拖动事件
  useEffect(() => {
      const handleMouseMove = (e: MouseEvent) => {
          if (!isResizing.current) return;
          const newWidth = window.innerWidth - e.clientX;
          if (newWidth > 280 && newWidth < 800) { // 限制最小/最大宽度
  ...
  return (
    <aside
      className="relative backdrop-blur-xl bg-card/60 border-l border-border/50 overflow-y-auto shadow-2xl shadow-black/20"
    ...
    >
      ...
      <div className="p-5 space-y-5">
          ...
          <div className="bg-muted/20 rounded-xl border border-border/30 relative overflow-hidden backdrop-blur-sm">
            {thumbnailType === "image" && thumbnailUrl ? (
              ...
              <div className="aspect-square flex items-center justify-center text-center text-muted-foreground">
                <div>
                  <div className="text-[11px] mb-2">检测影像/技术文档预览</div>
        ...
        <div>
          <h4 className="text-[14px] font-medium mb-3 text-blue">来源信息: </h4>
          <div className="bg-gradient-to-br from-muted/30 to-muted/20 rounded-xl p-4 space-y-3 text-[12px] border border-border/30 backdrop-blur-sm shadow-lg shadow-black/5">
            <div className="flex gap-3">
              <span className="text-gray-500 w-16 flex-shrink-0">文件: </span>
              <span className="flex-1 break-all text-white font-medium">{fileName}</span>
            </div>
            <div className="flex gap-3">
              <span className="text-gray-500 w-16 flex-shrink-0">路径: </span>
              <span className="flex-1 break-all text-[11px] text-gray-300">{filePath}</span>
            </div>
            <div className="flex gap-3">
              <span className="text-gray-500 w-16 flex-shrink-0">版本: </span>
              <span className="flex-1 text-gray-200">{version}</span>
            </div>
            <div className="flex gap-3">
              <span className="text-gray-500 w-16 flex-shrink-0">页码: </span>
          ...
          <div className="space-y-2">
            {structuredData.length > 0 ? (
              ...
              <div className="bg-muted/20 rounded-lg px-3 py-3 text-[12px] border border-border/30 backdrop-blur-sm text-center">
                <span className="text-gray-500">暂无结构化信息</span>
              </div>
            )}
          </div>
      ...
      <div
          ...
          className="absolute left-0 top-0 h-full w-1 cursor-ew-resize bg-transparent hover:bg-primary/30 transition-colors"
        ></div>
